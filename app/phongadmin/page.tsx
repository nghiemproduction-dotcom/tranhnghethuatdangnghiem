...
import { 
    Users, ShoppingBag, BarChart3, ChevronLeft, ChevronRight, Database 
} from 'lucide-react';
...
const ADMIN_FUNCTIONS = [
    ...
    { id: 'analytics', label: 'DOANH THU', icon: BarChart3 },
    { id: 'data', label: 'DỮ LIỆU', icon: Database }, // Thêm ví dụ để test scroll
];
...
export default function PhongAdminPage() {
    ...
    const scrollTabs = (direction: 'left' | 'right') => {
        if (tabsRef.current) {
            const scrollAmount = 150; // Giảm một chút cho mượt trên mobile
    ...
    return (
        ...
        >
            ...
            {/* Tinh chỉnh px-2 gap-2 cho mobile, px-4 gap-4 cho desktop */}
            <div className="flex-none z-30 w-full h-[60px] bg-[#080808] border-b border-[#C69C6D]/30 shadow-[0_5px_15px_rgba(0,0,0,0.8)] flex items-center px-2 gap-2 md:px-4 md:gap-4">
                ...
                <div className="shrink-0 bg-[#C69C6D] text-black px-4 md:px-6 py-2 rounded-l-lg rounded-r-2xl transform skew-x-[-10deg] shadow-[0_0_15px_rgba(198,156,109,0.5)] border-r-4 border-white/20">
                    <h1 className="text-xs md:text-base font-black uppercase tracking-[0.2em] skew-x-[10deg] whitespace-nowrap">
                        PHÒNG ADMIN
                    </h1>
                </div>
                ...
                <div className="flex-1 flex items-center min-w-0 gap-1 md:gap-2">
                    {/* Nút Trái - HIỆN TRÊN MỌI THIẾT BỊ */}
                    <button onClick={() => scrollTabs('left')} className="p-1 text-[#C69C6D] hover:bg-white/10 rounded shrink-0 active:scale-95 transition-transform">
                        <ChevronLeft size={20}/>
                    </button>
                        ...
                        {ADMIN_FUNCTIONS.map((func) => {
                            ...
                            return (
                                <button 
                                    ...
                                    className={`flex items-center gap-1.5 md:gap-2 text-[10px] md:text-[11px] font-bold uppercase px-3 md:px-5 py-2 rounded transition-all whitespace-nowrap border clip-path-game
                                        ${isActive 
                    ...
                    {/* Nút Phải - HIỆN TRÊN MỌI THIẾT BỊ */}
                    <button onClick={() => scrollTabs('right')} className="p-1 text-[#C69C6D] hover:bg-white/10 rounded shrink-0 active:scale-95 transition-transform">
            ...
            <div className="flex-1 w-full relative overflow-hidden bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] bg-[#050505]">
                ...
                <div className="absolute inset-0 z-10">
                    <div className="w-full h-full flex flex-col relative">
                        {/* Admin được phép xóa nhân sự */}
                        {activeFunction === 'hr' && <NhanSuManager allowDelete={true} />}
                        {activeFunction === 'orders' && <DonHangManager />}
                        
                        {(activeFunction === 'analytics' || activeFunction === 'data') && (
                            <div className="flex items-center justify-center h-full text-white/30 font-black uppercase tracking-widest text-xl">
