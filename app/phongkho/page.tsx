...
import VatTuManager from './VatTuManager';

const KHO_FUNCTIONS = [
    { id: 'vattu', label: 'VẬT TƯ & HÀNG HÓA', icon: Package },
    { id: 'nhapkho', label: 'NHẬP KHO', icon: Archive },
    { id: 'thongke', label: 'BÁO CÁO TỒN', icon: BarChart3 },
];

export default function PhongKho() {
    const { user: contextUser, loading: contextLoading } = useUser();
    const [authLoading, setAuthLoading] = useState(true);
    const [activeFunction, setActiveFunction] = useState<string>('vattu');
    ...
    return (
        <KhungTrangChuan nguoiDung={displayUser} loiChao="QUẢN LÝ KHO VẬN" contentClassName="flex flex-col h-screen pt-[70px] pb-0 px-0 overflow-hidden bg-[#050505]">
            ...
            <div className="flex-1 w-full relative overflow-hidden bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] bg-[#050505]">
                ...
                <div className="absolute inset-0 z-10">
                    <div className="w-full h-full flex flex-col relative">
                        ...
                        {activeFunction !== 'vattu' && <div className="h-full flex items-center justify-center text-white/30 font-bold uppercase">Chức năng đang phát triển</div>}
                    </div>
                </div>
            </div>
        </KhungTrangChuan>
