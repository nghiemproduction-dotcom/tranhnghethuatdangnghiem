...
import { ShieldCheck, Briefcase, Hammer, Clock, Users, TrendingUp, Lock, ChevronRight } from 'lucide-react';
...
export default function ModalPhongBan({ isOpen, onClose, currentUser }: Props) {
    ...
    const isSystemAdmin = myRole === 'admin_system' || myRole.includes('admin') || myRole.includes('boss') || myRole.includes('sep');
    ...
    const DEPARTMENTS = [
        ...
        { 
            ...
            allowedRoles: ['admin', 'adminsystem', 'quanly', 'manager', 'tho', 'thosanxuat', 'sales', 'kinhdoanh'], 
            href: '/phongsales' 
        },
        { 
            id: 'parttime', name: 'Phòng PartTime', 
            icon: <Clock className={iconClass} />, 
            color: 'text-purple-500',
            desc: 'Nhân sự bán thời gian', 
            allowedRoles: ['admin', 'adminsystem', 'quanly', 'manager', 'tho', 'thosanxuat', 'parttime'], 
            href: '/phongparttime' 
        },
        { 
            ...
            allowedRoles: ['admin', 'adminsystem', 'quanly', 'manager', 'tho', 'thosanxuat', 'congtacvien', 'ctv'], 
            href: '/phongctv' 
        },
    ];
                        ...
                        return (
                            <div key={dept.id} className={`relative group border rounded-xl p-[0.5vh] transition-all duration-300 ${canAccess ? 'border-[#8B5E3C]/30 hover:border-[#C69C6D] bg-[#1a120f]/80' : 'border-white/5 bg-black/40 opacity-50 grayscale'}`}>
                                ...
                                <div className="flex items-center gap-[3vw] p-[2vw] rounded-lg h-full">
                                    ...
                                    <div className="pr-2">
                                        {canAccess ? (
                                            <ChevronRight className="w-[clamp(16px,4vw,20px)] h-[clamp(16px,4vw,20px)] text-[#8B5E3C] group-hover:text-[#C69C6D] group-hover:translate-x-1 transition-transform" />
                                        ) : (
                                            <Lock className="w-[clamp(16px,4vw,20px)] h-[clamp(16px,4vw,20px)] text-gray-600" />
                                        )}
                                    </div>
                                </div>
                            </div>
