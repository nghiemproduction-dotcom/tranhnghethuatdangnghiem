...
// Chỉ import hàm logic tìm kiếm, KHÔNG import Modal
...
export default function Level2_DanhSachModal({ isOpen, onClose, config }: Props) {
    ...
    // Level 3 (Chi tiết) - Cái này vẫn phải giữ vì nó liên quan trực tiếp đến item click
    const [isLevel3Open, setIsLevel3Open] = useState(false);
    const [selectedItem, setSelectedItem] = useState<any>(null);
    
    // 🟢 ĐÃ XÓA TOÀN BỘ STATE SHOW MODAL (showSearchModal, showPermissionModal...)
    ...
    const handleRefresh = async () => {
        setLoading(true);
        setTimeout(() => fetchData(page, activeTab, search), 500);
    };

    const handleSavePermission = async (newConfig: ModuleConfig) => {
        try {
            await supabase.from('cau_hinh_modules').update({ config_json: newConfig }).eq('module_id', config.id);
            alert("Đã lưu phân quyền thành công!");
        } catch (e) { alert("Lỗi lưu phân quyền"); }
    };
    ...
    if (columns.length === 0 && data.length > 0) {
        columns = Object.keys(data[0]).map(k => ({ key: k, label: k, kieuDuLieu: 'text', hienThiList: true, hienThiDetail: true, tuDong: false, batBuoc: false }));
    }
    ...
    return (
        <div className="fixed top-0 left-0 right-0 bottom-[clamp(65px,16vw,85px)] z-[2200] bg-[#0a0807] flex flex-col animate-in slide-in-from-right-10 duration-300 shadow-2xl">
                ...
                <div className="flex flex-col h-full relative overflow-hidden">
                    <div className="shrink-0 bg-[#161210] border-b border-[#8B5E3C]/20 px-4 py-2 z-40">
                        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                            ...
                            {tabOptions.map((pos) => (
                                <button key={pos} onClick={() => handleTabChange(pos)} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase whitespace-nowrap border ${activeTab === pos ? 'bg-[#C69C6D] text-[#0a0807] border-[#C69C6D]' : 'bg-transparent text-[#8B5E3C] border-[#8B5E3C]/20 hover:bg-[#C69C6D]/10'}`}>{pos}</button>
                            ))}
                        </div>
                    </div>
                    {search && (
                        <div className="shrink-0 px-4 py-2 bg-[#161210] border-b border-[#8B5E3C]/10 flex justify-center">
                            <span className="px-4 py-1 bg-[#C69C6D]/20 text-[#C69C6D] border border-[#C69C6D] rounded-full text-xs font-bold uppercase flex items-center gap-2">
                                <Search size={12}/> Kết quả cho: "{search}" <button onClick={() => handleSearch('')}><X size={12} className="hover:text-white"/></button>
                            </span>
                        </div>
                    )}
                    <div className="flex-1 relative overflow-hidden">
                        ...
                        {data.length === 0 && !loading ? (
                            <div className="flex flex-col items-center justify-center h-full text-[#8B5E3C]"><p>Chưa có dữ liệu nào.</p></div>
                        ) : (
                            viewMode === 'table' ? (
                                <TableView data={data} columns={columns} page={page} itemsPerPage={ITEMS_PER_PAGE} onRowClick={(item) => { setSelectedItem(item); setIsLevel3Open(true); }}/>
                            ) : (
                                <CardView data={data} columns={columns} imgCol={imgCol} titleCol={titleCol} canEdit={true} onRowClick={(item) => { setSelectedItem(item); setIsLevel3Open(true); }} />
                            )
                        )}
                    </div>
                </div>
            ...
            {/* 🟢 NUT CHỨC NĂNG - QUẢN LÝ TOÀN BỘ MODAL CON */}
            <NutChucNang 
                config={config}
                canAdd={canAdd}
                canConfig={canConfig}
                viewMode={viewMode} 
                onToggleView={() => setViewMode(prev => prev === 'card' ? 'table' : 'card')} 
                onAdd={() => { setSelectedItem(null); setIsLevel3Open(true); }}
                onRefresh={handleRefresh}
                onClose={onClose}
                
                // Truyền Callback cho các nút đặc biệt bên trong
                onSearchData={handleSearch}
                currentSearch={search}
                onSaveConfig={handleSavePermission}
            />

            <Level3_FormChiTiet isOpen={isLevel3Open} onClose={() => setIsLevel3Open(false)} onSuccess={() => fetchData(page)} config={config} initialData={selectedItem} userRole={userRole} />
        </div>
    );
}