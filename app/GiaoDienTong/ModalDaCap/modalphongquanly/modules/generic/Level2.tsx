...
import KanbanView from '../quanlynhansu/Level2/KanbanView';
...
export default function Level2_Generic({ isOpen, onClose, config, onOpenDetail, isEmbedded = false, extraFilter }: Props) {
    ...
    const [groupByCol, setGroupByCol] = useState(getSmartGroupByColumn());
        ...
        <div className={`flex flex-col h-full bg-[#0F0C0B] ${isEmbedded ? '' : 'animate-in slide-in-from-right-10 duration-300'}`}>
            ...
            {!isEmbedded && !isLevel3Open && (
                <NutChucNang 
                    ...
                    onSearchData={(k) => { setSearch(k); setPage(1); fetchData(1, activeTab, k); }} currentSearch={search} 
                    onSaveConfig={async () => {}} 
                />
            )}
        </div>
