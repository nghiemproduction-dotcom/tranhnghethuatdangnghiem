...
export default function NhanSuChucNang({ 
    ...
    const filteredList = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            console.log('[NhanSu] searchTerm:', searchTerm, '| term (non-accent):', term);
            result = result.filter(i => {
                ...
                console.log('[NhanSu] Checking:', i.ho_ten, '| hoTen normalized:', hoTen, '| match:', match);
                return match;
            });
        }
                            ...
                            >
                                ...
                                <div className="p-4 flex items-center gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-[#C69C6D] truncate">{item.vi_tri}</p>
                    ...
                    <div className="p-4">
                        {detailTab === 'hoso' && (
                            <div className="space-y-3">
                                ...
                                <InfoRow icon={Briefcase} label="VỊ TRÍ" value={selectedItem.vi_tri} />
                            </div>
                        )}
                        {detailTab === 'luong' && (
                            <div className="space-y-3">
                                <InfoRow icon={Banknote} label="LƯƠNG THÁNG" value={formatMoney(selectedItem.luong_thang)} highlight />
                                <InfoRow icon={Banknote} label="LƯƠNG THEO GIỜ" value={formatMoney(selectedItem.luong_theo_gio)} />
                                <InfoRow icon={Banknote} label="THƯỞNG DOANH SỐ" value={`${selectedItem.thuong_doanh_thu || 0}%`} />
                            </div>
                        )}
                        {detailTab === 'nganhang' && (
                            <div className="space-y-3">
                                <InfoRow icon={Banknote} label="NGÂN HÀNG" value={selectedItem.ngan_hang} />
                                <InfoRow icon={Banknote} label="SỐ TÀI KHOẢN" value={selectedItem.so_tai_khoan} />
                            </div>
                        )}
                    </div>
                </KhungChiTiet>
                    ...
                    <div className="space-y-4">
                        ...
                        <FormField label="Lương tháng (VNĐ)">
                            <input
                                ...
                                onChange={e => setFormData({ ...formData, luong_thang: Number(e.target.value) })}
                                className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder:text-white/30 focus:border-[#C69C6D]/50 focus:outline-none"
                                placeholder="0"
                            />
                        </FormField>
...
function InfoRow({ icon: Icon, label, value, highlight }: { icon: any; label: string; value?: string; highlight?: boolean }) {
    return (
        <div className="flex items-center gap-3 p-3 rounded-xl bg-[#151515] border border-white/5">
            <div className={`p-2 rounded-lg ${highlight ? 'bg-[#C69C6D]/20 text-[#C69C6D]' : 'bg-black text-[#C69C6D]'}`}>
                <Icon size={16} />
            </div>
            <div className="flex-1 min-w-0">
                <p className="text-[9px] font-black uppercase tracking-wider text-white/40 mb-0.5">{label}</p>
                <p className={`text-sm font-bold truncate ${highlight ? 'text-[#C69C6D]' : 'text-gray-200'}`}>{value || '---'}</p>
            </div>
        </div>
    );
}
