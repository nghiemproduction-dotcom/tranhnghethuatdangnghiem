...
interface Props {
  tenPhong: string;
  // ✅ [SỬA] Thêm dấu ? để không bắt buộc (vì layout gọi không truyền)
...
export default function ThanhPhongChucNang({
  tenPhong,
  functions = [], // Giá trị mặc định là mảng rỗng
  activeFunction,
  onFunctionChange,
  userRole,
}: Props) {
  ...
  // Nếu không có danh sách chức năng thì chỉ hiện tên phòng
              ...
              {functions.map((func) => {
                ...
                return (
                  <button
                    key={func.id}
                    onClick={() => {
                      if (onFunctionChange) onFunctionChange(func.id);
